...
class OneDiceActivity : AppCompatActivity(), View.OnClickListener, SensorEventListener {
    ...
    val SHAKE_SKIP_TIME: Int = 2000
    val SHAKE_THRESHOLD_GRAVITY: Float = 50.7F
        ...
        if (e?.sensor!!.type == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (gabOfTime > SHAKE_SKIP_TIME) {
                ...
                if (speed > SHAKE_THRESHOLD_GRAVITY) {
                    // 이벤트발생!!
                    Log.d(this.javaClass.name, "Shake 발생")
                    setDice()
                }
